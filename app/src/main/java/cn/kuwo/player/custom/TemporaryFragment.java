 ...
 */
public class TemporaryFragment  extends DialogFragment implements View.OnClickListener{
    ...
    public TemporaryFragment(Context context,ProductBean productBean,Boolean isEdit,String barcode) {
        ...
        if (productBean.getComboMenu().length()>0&&productBean.getComboMenu()!=null) isComobo=true;
        if (barcode.length()==18) isWeight=true;
    }

    @SuppressLint("ValidFragment")
    public TemporaryFragment(Context context,ProductBean productBean,Boolean isEdit,Object order,int orderIndex){
        this.context = context;
        this.productBean = productBean;
        this.isEdit = isEdit;
        this.order = order;
                ...
                if (isEdit) {
                    ...
                    if (Double.parseDouble(tvNumber.getText().toString().trim()) >= 2) {
                        ...
                        if (isWeight) {
                            ...
                            price.setText("￥" + MyUtils.formatDouble(commodityNumber * productBean.getPrice()+sideDishPrice));
                        }
                    }
                }
    ...
    private void setSideDish() {
        if (isEdit){
            ...
            RealmList<SideDishEntity> sidedish = productBean.getSidedish();
            if (sidedish.size()>0){
    ...
    private void showSideDishBottomSheetList() {
        final RealmList<SideDishEntity> sidedish = productBean.getSidedish();
        ...
        bs.setOnSheetItemClickListener(new QMUIBottomSheet.BottomListSheetBuilder.OnSheetItemClickListener() {
            ...
            public void onClick(QMUIBottomSheet dialog, View itemView, int position, String tag) {
                ...
                if (isWeight) {
                    price.setText("￥" + MyUtils.formatDouble(ProductUtil.calCommodityMoney(barcode)+totalDishPrice));
                } else {
                    price.setText("￥" + MyUtils.formatDouble(productBean.getPrice()+totalDishPrice));
                }
            }
        }).build().show();
    }
